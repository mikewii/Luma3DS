...
static u32 g_patchIrSync = 0;

static void IR__DoUndoPatchs(Handle processHandle, bool doPatch)
{
    ...
    */
    ...
    // Apply patches
    IR__DoUndoPatchs(processHandle, doPatch);
}

static Result  IR__GetProcessHandle(Handle* processHandle)
{
    return OpenProcessByName("ir", processHandle);
}

static Handle  IR__UnlockThreads(Handle processHandle)
{
    if (processHandle || R_SUCCEEDED(IR__GetProcessHandle(&processHandle)))
        // Unlock all ir threads
        svcControlProcess(processHandle, PROCESSOP_SCHEDULE_THREADS, 0, 0);
    return processHandle;
}

static Handle  IR__LockThreads(Handle processHandle)
{
    if (processHandle || R_SUCCEEDED(IR__GetProcessHandle(&processHandle)))
        // Lock all ir threads
...
{
    if (!g_patchIrSync || --g_patchIrSync > 0)
        return;

    Handle processHandle = IR__LockThreads(0); ///< Lock threads as extra safety to prevent prefetch abort
    if (!processHandle)
    {
        ++g_patchIrSync;
        return;
    }
    IR__DoUndoPatchs(processHandle, false);
    IR__UnlockThreads(processHandle);
...
void    IR__Patch(void)
{
    ...
    IR__DoUndoPatchs(processHandle, true);
    IR__UnlockThreads(processHandle);
    svcCloseHandle(processHandle);
}

void    IR__HandleSleep(bool isSleeping)
{
    if (!g_patchIrSync)
        return; ///< nothing to do if IR patch not applied
    ...
    IR__DoUndoPatchs(processHandle, !isSleeping); ///< Unpatch if sleeping and repatch when exiting sleep mode
    IR__UnlockThreads(processHandle);
    svcCloseHandle(processHandle);
}
